...
export default function StudentsResults() {
  ...
  const calculateCurrentStudentAverage = () => {
    ...
    clone = clone.map( student => {
      ...
      student.avg = avg;
      student.admission = avg >= 10 ? "ACCEPTED" : "REJECTED"
  ...
  useEffect(() => {}, [data]);
  useEffect(() => { calculateCurrentStudentAverage() }, [currentStudents]);
  useEffect(() => {selectCurrentStudents()}, [promo]);
  useEffect(() => {selectCurrentStudents()}, [semester]);
  useEffect(() => {
